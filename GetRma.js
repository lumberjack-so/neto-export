...
serve(async () => {
	...
	try {
		...
		const rmas = response.Rma || [];

		console.log(`[${entityType}] Received ${rmas.length} records`);

		let recordsInserted = 0;

		if (rmas.length > 0) {
			// Transform and upsert data
			const transformedData = transformData(endpoint, { Rma: rmas });
		...
		const hasMorePages = rmas.length === syncParams.page_size;
