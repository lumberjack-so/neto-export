...
serve(async () => {
	...
	try {
		...
		console.log(`[${entityType}] Received ${customers.length} records`);

		let recordsInserted = 0;

		if (customers.length > 0) {
			// Transform and upsert data
			const transformedData = transformData(endpoint, { Customer: customers });
		...
		const hasMorePages = customers.length === syncParams.page_size;
		...
		const response_data = {
			...
			records_fetched: customers.length,
