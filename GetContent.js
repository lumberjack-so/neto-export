...
serve(async () => {
	...
	try {
		...
		console.log(`[${entityType}] Received ${contents.length} records`);

		let recordsInserted = 0;

		if (contents.length > 0) {
			// Transform and upsert data
			const transformedData = transformData(endpoint, { Content: contents });
		...
		const hasMorePages = contents.length === syncParams.page_size;
		...
		const response_data = {
			...
			records_fetched: contents.length,
