...
serve(async () => {
	...
	try {
		...
		console.log(`[${entityType}] Received ${suppliers.length} records`);

		let recordsInserted = 0;

		if (suppliers.length > 0) {
			// Transform and upsert data
			const transformedData = transformData(endpoint, { Supplier: suppliers });
		...
		const hasMorePages = suppliers.length === syncParams.page_size;
		...
		const response_data = {
			...
			records_fetched: suppliers.length,
