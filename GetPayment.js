...
serve(async () => {
	...
	try {
		...
		console.log(`[${entityType}] Received ${payments.length} records`);

		let recordsInserted = 0;

		if (payments.length > 0) {
			// Transform and upsert data
			const transformedData = transformData(endpoint, { Payment: payments });
		...
		const hasMorePages = payments.length === syncParams.page_size;
		...
		const response_data = {
			...
			records_fetched: payments.length,
